...
if [[ ! $VERSION =~ ^v[[:digit:]]+\.[[:digit:]]+\.[[:digit:]]+$ ]]; then
  echo "version format error: $VERSION, HISTORY.md first line must be '## v0.0.1 <commentstring>' format "
  exit
fi

COMMENT=`head -n1 HISTORY.md | awk '{$1="";$2="";sub("  "," ");print $0;}'`
if [[ $COMMENT == "" || $COMMENT == " " ]]; then
  echo "no comment: HISTORY.md first line must be '## v0.0.1 <commentstring>' format "
  exit
fi
